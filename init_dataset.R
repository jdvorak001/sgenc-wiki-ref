...
init_dataset <- function(source_folder){
  # takes source folder name and merges all files inside it into a single dataset
  ...
  # update date_harvested column to be Date type
  ...
  dataset <- dataset %>% filter(!grepl("^Wikipedista:", title))
  
  # returns dataset, use variable_name <- init_dataset("./source") to add into workspace
  return(dataset)
}

init_articles_edited <-function(){
  # load the edited articles from csv
  articles_edited <- read_csv("rstudio_import/articles_edited_list.csv")
  return(articles_edited)
}

add_page_id_to_ae <- function(ae,ds){
  # add page id's using title provided in dataset
